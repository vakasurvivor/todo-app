...
async function render() {
  ...
  const tasks = await getAllTasks(); // 全件取得（READ）
...
async function handleAddTaskFormSubmit(e) {
  ...
  const id = await addTask(text); // 追加（CREATE）
  const task = await getTask(id); // 取得（READ）
...
async function handleTaskListClick(e) {
  ...
  async function handleEditBtnClick(btnEl) {
    ...
    async function finishEdit() {
      ...
      if (newText && newText !== oldText) {
        const res = await updateTask(id, { text: newText }); // 更新（UPDATE）
  ...
  async function handleDeleteBtnClick(btnEl) {
    ...
    if (response) {
      ...
      const res = await deleteTask(id); // 削除（DELETE）
...
async function handleTaskListChange(e) {
  if (e.target.matches("input.task-completed")) {
    ...
    const res = await updateTask(id, { is_completed: isCompleted }); // 更新（UPDATE）
    if (res) {
    }
  }
}
