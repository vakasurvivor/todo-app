import { useTransition } from "react";
import { cn } from "@/utils/cn";
import { actionUpdateTask } from "@/libs/actions";
import { toast } from "sonner";
import { useTasksContext } from "../../tasks-context";

type TaskCheckboxProps = {
  id: number;
  isCompleted: boolean;
};

export default function Checkbox({ id, isCompleted }: TaskCheckboxProps) {
  const { setTasks } = useTasksContext();
  const [isPending, startTransition] = useTransition();

  async function handleChange() {
    startTransition(async () => {
      const res = await actionUpdateTask(id, { isCompleted: !isCompleted });
      if (!res.success && "error" in res) {
        toast.error(res.error.message);
      } else if (!res.success) {
        toast.error("タスクの変更に失敗しました。");
      }

      startTransition(() => {
        setTasks((prev) =>
          prev.map((t) =>
            t.id === id ? { ...t, isCompleted: !t.isCompleted } : t
          )
        );
      });
    });
  }

  return (
    <label
      className={cn(
        "grid w-fit",
        "[&:has(input:checked)+span]:text-muted-foreground",
        "[&:has(input:checked)+span]:transition-[text-decoration]",
        "[&:has(input:checked)+span]:[text-decoration:line-through_1.3px_var(--color-muted-foreground)]"
      )}
    >
      <input
        className={cn(
          "relative cursor-pointer appearance-none ",
          "size-[22px] bg-white border border-border rounded-xs transition-colors",
          "hover:bg-gray-100 hover:border-ring",
          "before:content-[''] before:absolute before:block before:size-full before:inset-0 before:bg-gray-950",
          "before:opacity-0 before:transition-opacity before:ease-out",
          "before:mask-[url(/img/check.svg)] before:mask-no-repeat before:mask-contain",
          "checked:before:opacity-100 checked:before:transition-opacity ease-out"
        )}
        name="task-completed"
        type="checkbox"
        checked={isCompleted}
        onChange={handleChange}
        disabled={isPending}
      />
    </label>
  );
}
